                    ...
                    if ev.type == pygame.MOUSEBUTTONUP and is_card_drag:
                        ...
                        if isinstance(card, Card) and not card.is_in_board \
                            ...
                            if (left_row == card.row and card.col == hint_col) or (right_row == card.row and card.col == hint_col):
                                ...
                                latest_left_row, latest_right_row, latest_hint_col, \
                                    latest_pivot_card_left, latest_pivot_card_right = self.board.get_hint_tile()

                                msg = {
                                    'status': 'send_card',
                                    'card': card.serialize_data(),
                                    'pivot_card_left': latest_pivot_card_left.serialize_data(),
                                    'pivot_card_right': latest_pivot_card_right.serialize_data(),
                                }
                                self.network.send_to_server(msg)
                            else:
